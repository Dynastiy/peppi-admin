<template>
          ...
          <div class="tw-flex tw-justify-between">
            <div>
              <h6 class="tw-font-semibold tw-mb-0">
                {{ "Order ID:" }}
                <span v-if="item.order" class="tw-uppercase">{{
                  "#" + item.order.id.split("-").join("").slice(0, 5)
                }}</span>
              </h6>
  ...
  export default {
    ...
    methods: {
      getOrder() {
        this.busy = true;
        this.$request(`admin/payments/${this.id}`)
          .then((res) => {
            let resPayload = res.data;
            console.log(resPayload);
            this.item = resPayload.payment;
            this.products = resPayload.payment.order.products;
            this.busy = false;
          })
          .catch((err) => {
            console.log(err);
    ...
    computed: {
      ...
      pickupData() {
        return pick(this.item.order.pickup_location, this.pickupDisplay);
      },
      txnData() {
        return pick(this.item, this.txnDisplay);
      },
      costDetails() {
        let data = {
          total_quantity: Number(this.item.order.total_quantity).toLocaleString(),
          total_amount: Number(this.item.order.total_amount).toLocaleString("en-US", {
            style: "currency",
            currency: "NGN",
          }),
        };
